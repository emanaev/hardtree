<template>
  <svg v-bind:width="totalWidth" v-bind:height="totalHeight">
    ...
    <g v-for="node in graph.nodes">
      <Node v-bind:node="node" v-bind:y="node.x*50" v-bind:x="node.y*110+50"  v-bind:primary="!graph.isCircleNode(node)" v-on:generate-forward="$emit('generate-forward', node)"/>
    </g>
    <g v-for="link in graph.links" >
      ...
      <LinkNds v-bind:link="link" v-bind:reversed="graph.show_reversed" />
    </g>
  </svg>
</template>
